...
import com.example.fishing.filter.FormAuthenticationProvider;
...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
	...
	protected void configure(HttpSecurity http) throws Exception {
		//@formatter:off
		http.authorizeRequests().antMatchers("/", "/login", "/logout-complete", "/users/new", "/user").permitAll().anyRequest().authenticated()
		// ログアウト処理
		...
		// form
		.and().formLogin().loginPage("/login").defaultSuccessUrl("/auth").failureUrl("/login-failure")
		.permitAll();
		//@formatter:on
	}
	@Override
